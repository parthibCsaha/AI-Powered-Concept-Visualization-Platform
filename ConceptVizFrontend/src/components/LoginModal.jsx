...
export default function LoginModal({ isOpen, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isLogin) {
        ...
        response = await authService.signup(formData.email, formData.password, formData.name);
      }

      if (response.token) {
        const user = { email: response.email, name: response.name };
        authService.storeAuth(response.token, user);
        setUser(user);
        onSuccess?.();
        onClose();
      } else {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
      <div className="card-modern max-w-md w-full animate-slide-up">
